...
public class CurrencyConversionController
{
	...
	// Above method uses RestTemplate to call another service
	// Below method uses Feign to call another service
	...
	public CurrencyConversionBean convertCurrencyFeign(@PathVariable String from, @PathVariable String to, @PathVariable BigDecimal quantity)
	{
		...
		// Logging
		logger.info("Logging Currency Conversion Service : {}", response);
		// Logging
		return new CurrencyConversionBean(response.getId(), from, to,
																	response.getConversionMultiple(), quantity,
																	quantity.multiply(response.getConversionMultiple()), response.getPort());
